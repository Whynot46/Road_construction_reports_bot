...
def is_natural_digit(value: str) -> bool:
    try:
        value = int(value)
        return value >= 0
    except ValueError:
...
async def set_route_breakdown(message: Message, state: FSMContext, message_text: str):
    if is_natural_digit(message_text) or is_float(message_text) or message_text == "":
...
async def set_quarries_construction_quantity(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "quarries_construction_quantity" not in data or data["quarries_construction_quantity"] == "":
        await state.update_data(quarries_construction_quantity=message_text)
    else:
        await state.update_data(quarries_construction_quantity=data["quarries_construction_quantity"] + "\n" + message_text)
    await state.set_state(Preparatory_steps.add_material_choice)
    await message.answer("Добавить ещё материал или перейти к следующему вопросу?", reply_markup=await kb.get_add_material_keyboard())


@exception_decorator
@router.message(Preparatory_steps.add_material_choice)
async def set_add_material_choice(message: Message, state: FSMContext):
    if message.text == "Добавить материал":
        ...
        await state.set_state(Preparatory_steps.add_material_choice)
        await message.answer("Неизвестная команда", reply_markup=await kb.get_add_material_keyboard())
...
async def set_excavations_development(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "excavations_development" not in data or data["excavations_development"] == "":
        await state.update_data(excavations_development=message_text)
    else:
        await state.update_data(excavations_development=data["excavations_development"] + "\n" + message_text)
...
async def set_excavations_development_quantity(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "excavations_development_quantity" not in data or data["excavations_development_quantity"] == "":
        await state.update_data(excavations_development_quantity=message_text)
    else:
        await state.update_data(excavations_development_quantity=data["excavations_development_quantity"] + "\n" + message_text)
    await state.set_state(Earthworks_steps.add_work_choice)
    await message.answer("Добавить ещё работы или перейти к следующему вопросу?", reply_markup=await kb.get_add_work_keyboard())


@exception_decorator
@router.message(Earthworks_steps.add_work_choice)
async def set_add_work_choice(message: Message, state: FSMContext):
    if message.text == "Добавить ещё работы":
        await state.set_state(Earthworks_steps.excavations_development)
        await message.answer("Разработка выемок и возведение насыпей. Укажите вид работ.", reply_markup=await kb.get_none_work_keyboard())
    elif message.text == "Перейти к следующему вопросу":
        ...
        await state.set_state(Earthworks_steps.add_work_choice)
        await message.answer("Неизвестная команда", reply_markup=await kb.get_add_work_keyboard())
...
async def set_work_type(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "work_type" not in data or data["work_type"] == "":
        await state.update_data(work_type=message_text)
    else:
        await state.update_data(work_type=data["work_type"] + "\n" + message_text)
...
async def set_work_scope(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "work_scope" not in data or data["work_scope"] == "":
        await state.update_data(work_scope=message_text)
    else:
        await state.update_data(work_scope=data["work_scope"] + "\n" + message_text)
    await state.set_state(Artificial_structures_steps.add_work_choice)
    await message.answer("Добавить ещё работы или перейти к следующему вопросу?", reply_markup=await kb.get_add_work_keyboard())


@exception_decorator
@router.message(Artificial_structures_steps.add_work_choice)
async def set_add_work_choice(message: Message, state: FSMContext):
    if message.text == "Добавить ещё работы":
        await state.set_state(Artificial_structures_steps.work_type)
        await message.answer("Укажите вид работ.", reply_markup=await kb.get_none_work_keyboard())
...
async def set_other_works(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "other_works" not in data or data["other_works"] == "":
        await state.update_data(other_works=message_text)
    else:
        await state.update_data(other_works=data["other_works"] + "\n" + message_text)
    await state.set_state(Road_devices_steps.add_work_choice)
    await message.answer("Добавить ещё работы или перейти к следующему вопросу?", reply_markup=await kb.get_add_work_keyboard())


@exception_decorator
@router.message(Road_devices_steps.add_work_choice)
async def set_add_work_choice(message: Message, state: FSMContext):
    if message.text == "Добавить ещё работы":
        await state.set_state(Road_devices_steps.other_works)
        await message.answer("Напишите другую работу с объемом по обстановке дороги.", reply_markup=await kb.get_none_work_keyboard())
...
async def set_other_material(message: Message, state: FSMContext, message_text: str):
    data = await state.get_data()
    if "other_material" not in data or data["other_material"] == "":
        await state.update_data(other_material=message_text)
    else:
        await state.update_data(other_material=data["other_material"] + "\n" + message_text)
    await state.set_state(Material_consumption_report_steps.add_material_choice)
    await message.answer("Добавить ещё материал или перейти к следующему вопросу?", reply_markup=await kb.get_add_material_keyboard())


@exception_decorator
@router.message(Material_consumption_report_steps.add_material_choice)
async def set_add_material_choice(message: Message, state: FSMContext):
    if message.text == "Добавить материал":
        await state.set_state(Material_consumption_report_steps.other_material)
        await message.answer("Другие материалы. Виды и количество?", reply_markup=await kb.get_skip_keyboard())
