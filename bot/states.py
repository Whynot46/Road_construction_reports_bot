...
class Shift_steps(StatesGroup):
    shift = State() #смена
    stage = State() #этап работ


#Отчет по этапу "Подготовительные работы"
class Preparatory_steps(StatesGroup):
...
class Earthworks_steps(StatesGroup):
...
class Asphalt_steps(StatesGroup):
...
class Road_devices_steps(StatesGroup):
...
class material_consumption_report_steps(StatesGroup):
...
class People_and_equipment_report_steps(StatesGroup):
